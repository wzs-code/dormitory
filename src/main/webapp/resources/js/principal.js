...
//              alert("sss");
    ...
    $("#select_principal").click(function(){
        //得到input的值
        ...
        if(info==""){
            ...
            //设置linkToCart的href的值
            $("#select_principal").attr("href","/ssm2/selectPrincipal?principal="+info);
        }
...
var obj = document.getElementsByName("principalIds");//将复选框定义成一个jquery对象
...
function todelect() {
    ...
    $("input[name='principalIds']").each(function () {//获取复选框节点id
    ...
    if (All.checked) {
        ...
        $.ajax({//利用ajax发出请求
            url: 'http://localhost:8080/ssm2/deleteprincipals?principalIds=' + check_val, //向Controller里的deleteSelect传输ids
            type: "post",//post类型
            data:{"data":check_val},
            success: function (data) {//删除成功后，deleteMany会返回一个"ok";
                if (data == "ok") {
                    alert("删除成功！");//返回ok后弹出一个对话框。
                    location.href = "http://localhost:8080/ssm2/principal";//相当于刷新界面
                }
            }
        });
    }
...
function principal_update(principalId) {
    $.ajax({
        url: "http://localhost:8080/ssm2/update_principal"+'?principalId='+principalId,
        type: "post",
        data: {"principalId":principalId},
        success:function (data) {
            //传递过来的是json格式的字符串，需要解析为json对象
            var data = $.parseJSON(data);
            $('#principalId1').val(data.account.principalId);
            $('#principal1').val(data.account.principal);
            $('#dormitory1').val(data.account.dormitory);
            $('#principalSex1').val(data.account.principalSex);
            $('#principalTel1').val(data.account.principalTel);
            $('#duty1').val(data.account.duty);
        },
        error:function () {
            alert("Aja失败了");
        }
